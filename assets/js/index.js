...
;(function() {
  ...
  // links &&
  //   links.addEventListener('click', function(evet) {
  //     if (evet.target.tagName === 'IMG') {
  //       window.open(evet.target.alt)
  //     }
  //   })
  var barLinks = document.querySelectorAll('.home-bar a')
  if (barLinks) {
    for (var i = 0, len = barLinks.length; i < len; i++) {
      var ele = barLinks[i]
  ...
  homeBar && homeBar.addEventListener('click', function(e) {
    console.log(e)
    if (e.target.nodeName === 'LI') {
      location.href = e.target.children[0].href
    }
  })
  const markdownBody = querySelector('.markdown-body')
  markdownBody && markdownBody.addEventListener('click', function(e) {
